...
<body>
	...
	<div class="page">
		...
		<header class="sub">
			...
			<span class="score" style="margin-left:15%">
				score :
				<?php echo($_GET["score"]) ; 
                                $sc=0;
                                $sc=$_GET["score"];
			...
			<span class="new">
                        ...
                        <span class="new" style="margin-left:30%">
                            <?php 
                            $uidn="";
                            
                            $uidn=$_SESSION['u_id'];
                                $sqn="SELECT user.name, MAX(gamedata.score) FROM user JOIN gamedata WHERE user.u_id=$uidn AND gamedata.ug_id=$uidn AND gamedata.g_id=1";
                                $sqnc= mysqli_query($con, $sqn);
                                $s= mysqli_fetch_array($sqnc);
                                $name="";
                                $name=$s[0];
                                echo "Top scorer : $name";
                            ?>
